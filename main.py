...
sky = pygame.image.load("Assets/bg.png")
...
overlay = pygame.transform.scale(vignette, (worldx,worldy))

collect_fx = pygame.mixer.Sound('Assets/coin.wav')
collect_fx.set_volume(0.5)

jump_fx = pygame.mixer.Sound('Assets/jump.wav')
jump_fx.set_volume(0.5)

hurt_fx = pygame.mixer.Sound('Assets/hurt.mp3')
...
backwardx_wall = 250 #left side imaginary wall
forwardx_wall = worldx - 250 #right side imaginary wall


topy_wall = 350 #top side imaginary wall
bottomy_wall = worldy - 250 #bottom side imaginary wall
...
class Player(pygame.sprite.Sprite):
  def __init__(self, x, y):
    super().__init__()
    player_pic = pygame.image.load('Assets/catwalk.gif')
...
class Platform(pygame.sprite.Sprite):
  def __init__(self, x, y, main_group, sub_group, image_int):
    ...
    #load images
    if image_int == 1:
      grass_pic = pygame.image.load('Assets/floor1.png')
      ...
      box_pic = pygame.image.load('Assets/block.png')
    ...
    #ruby tiles
      ...
      ruby_pic = pygame.image.load('Assets/chicken.gif')
...
class Enemy(pygame.sprite.Sprite):
  def __init__(self, x, y, image_int):
    super().__init__()
    if image_int == 5:
      evil_blob = pygame.image.load('Assets/spike.png')
...
while play_game:
  ...
  if main_menu ==True: 
    ...
    # scroll the world moving up 
    if player.rect.y >= topy_wall:
      scroll = player.rect.y - topy_wall
      player.rect.y = topy_wall
    ...
    # scroll the world moving down
    if player.rect.y <= bottomy_wall:
      scroll = bottomy_wall - player.rect.y
      player.rect.y = bottomy_wall
  ...
  for event in pygame.event.get():
      ...
      if event.type == pygame.KEYDOWN:
        ...
        if event.key == pygame.K_UP or event.key == ord('w') or event.key == pygame.K_SPACE:
          player.jump() # replaced print jump
          jump_fx.play()
